...
df[string_show + "_" + metrics] = pd.to_numeric(
    df[string_show + "_" + metrics], errors="coerce"
)
...
x_tick_labels = df_sorted.apply(
    lambda row: (
        f"{row['pfa_scores']}__{row['use_extra_features']}__{row['selection_extra_features']}"
        if pd.notna(row["pfa_scores"])
        else row["method"]
    ),
    axis=1,
)
...
plt.bar(
    df_sorted.index,
    df_sorted["EW_UD_" + metrics],
    color=bar_colors,
    label="EW_UD (time2feat)",
    alpha=0.6,
)
...
plt.bar(
    df_sorted.index,
    df_sorted[string_show + "_" + metrics],
    color=bar_colors,
    label="UD (kshape and kmeans)",
    alpha=0.6,
)
...
if not kmeans_data.empty:
    plt.bar(
        kmeans_data.index,
        kmeans_data[string_show + "_" + metrics],
        color=bar_colors[kmeans_data.index],
        label=string_show + " (kmeans)",
        alpha=0.6,
    )
...
if not kshape_data.empty:
    plt.bar(
        kshape_data.index,
        kshape_data[string_show + "_" + metrics],
        color=bar_colors[kshape_data.index],
        label=string_show + " (kshape)",
        alpha=0.6,
    )
...
plt.ylabel("Values")
plt.title(
    "Bar Plot of EW_UD and "
    + string_show
    + " on Metric "
    + metrics
    + ". The cluster are "
    + str(df.iloc[0]["n_clusters_used"])
    + ". The value of t2f represents (pfa_scores, use_extra_features, selection_extra_features)"
)
